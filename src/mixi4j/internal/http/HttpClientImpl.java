...
public class HttpClientImpl extends HttpClientBase implements HttpClient, HttpResponseCode, java.io.Serializable {
    ...
    public HttpResponse post(String url, HttpParameter[] params) throws MixiException {
        return request(new HttpRequest(RequestMethod.POST, url, params, null, null));
    }

    public HttpResponse request(HttpRequest req) throws MixiException {
        int retriedCount;
        ...
        for (retriedCount = 0; retriedCount < retry; retriedCount++) {
            ...
            try {
                ...
                try {
                    ...
                    if(responseCode == HttpURLConnection.HTTP_UNAUTHORIZED){
            			...
            			if(responseHeader != null && responseHeader.indexOf("expired_token") != -1){
            				throw new MixiException("アクセストークンを最取得する必要がある, expired_token");
            			}

                    }
                    else if (responseCode < OK || (responseCode != FOUND && MULTIPLE_CHOICES <= responseCode)) {
                        if (responseCode == ENHANCE_YOUR_CLAIM ||
                                responseCode == BAD_REQUEST ||
                                responseCode < INTERNAL_SERVER_ERROR ||
                                retriedCount == CONF.getHttpRetryCount()) {
                            throw new MixiException(res.asString(), res);
                        }
                ...
                if (retriedCount == CONF.getHttpRetryCount()) {
                    throw new MixiException(ioe.getMessage(), ioe, responseCode);
                }
            }
            try {
...
//        }
      ...
      String authorizationHeader = req.getAuthorization().getOAuthToken().getToken();
      if(authorizationHeader == null){
    	  throw new AssertionError("access tokenがnull");
      }
		connection.addRequestProperty("Authorization", "OAuth " + authorizationHeader);
